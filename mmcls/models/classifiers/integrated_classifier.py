...
class IntegratedClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 init_cfg=None):
        super(IntegratedClassifier, self).__init__(init_cfg)
    ...
    def init_backbone(self, backbone):
        ...
        for item in backbone:
            self.backbones.append(build_backbone(item))

    def init_neck(self, neck):
        self.necks = ModuleList()
        for item in neck:
            self.necks.append(build_neck(item))

    def init_head(self, head):
        self.heads = ModuleList()
        for item in head:
            self.heads.append(build_head(item))
    ...
    def extract_feat(self, img, stage='neck', gt_label=None):
        ...
        for i in range(self.num_models):
            backbone, neck, head = self.backbones[i], self.necks[i], self.heads[i]
            x = backbone(img)  # ((B, C))
            if neck is not None:
                x = neck(x)
            x = head.forward_train(x, gt_label, stage='tensor')

            x = F.softmax(x, dim=1)
            scores.append(x)
        outs = [torch.cat(scores, dim=1)]
    ...
    def forward_train(self, img, gt_label, **kwargs):
        ...
        x = self.extract_feat(img, gt_label=gt_label)       # B, 3000

        losses = dict()
        loss = self.heads[self.num_models].forward_train(x, gt_label)
    ...
    def simple_test(self, img, img_metas=None, **kwargs):
        ...
        if isinstance(self.heads[self.num_models], MultiLabelClsHead):
            assert 'softmax' not in kwargs, (
                'Please use `sigmoid` instead of `softmax` '
                'in multi-label tasks.')
        res = self.heads[self.num_models].simple_test(x, **kwargs)
